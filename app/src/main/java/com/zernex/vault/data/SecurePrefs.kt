...
class SecurePrefs(context: Context) {
    ...
    /** Délai d'auto-verrouillage en ms (0 = immédiat au passage en arrière-plan). */
    var autoLockMs: Long
        get() = prefs.getLong(KEY_AUTO_LOCK, 0L)
        set(v) = prefs.edit().putLong(KEY_AUTO_LOCK, v).apply()
    ...
    var favoritesOnly: Boolean
        get() = prefs.getBoolean(KEY_FAV_ONLY, false)
        set(v) = prefs.edit().putBoolean(KEY_FAV_ONLY, v).apply()

    var moveOnImport: Boolean
        get() = prefs.getBoolean(KEY_MOVE_IMPORT, true)
        set(v) = prefs.edit().putBoolean(KEY_MOVE_IMPORT, v).apply()

    /** Catégorie filtrée (null = Tout). */
    var selectedCategory: VaultCategory?
        get() {
            val raw = prefs.getString(KEY_CATEGORY, null) ?: return null
            return runCatching { VaultCategory.valueOf(raw) }.getOrNull()
        }
        set(v) = prefs.edit().putString(KEY_CATEGORY, v?.name).apply()
    ...
    companion object {
        ...
        private const val KEY_FAV_ONLY = "favorites_only"
        private const val KEY_MOVE_IMPORT = "move_on_import"
